// Server/src/Controllers/SolutionSnapshotController.js
...
/**
 * Create SolutionSnapshot
 * Accepts multipart/form-data files OR body URLs for snapimg1..4
 */
export const createSolutionSnapshot = async (req, res) => {
  ...
  try {
    ...
    const payload = {
      counter1: norm(req.body.counter1),
      countertext1: norm(req.body.countertext1),
      counter2: norm(req.body.counter2),
      countertext2: norm(req.body.countertext2),
      counter3: norm(req.body.counter3),
      countertext3: norm(req.body.countertext3),
    ...
    const missing = [];
    if (!payload.counter1) missing.push("counter1");
    if (!payload.countertext1) missing.push("countertext1");
    if (!payload.counter2) missing.push("counter2");
    if (!payload.countertext2) missing.push("countertext2");
    if (!payload.counter3) missing.push("counter3");
    if (!payload.countertext3) missing.push("countertext3");
    ...
    const doc = new SolutionSnapshot(payload);
    await doc.save({ session });

    await session.commitTransaction();

    const result = await SolutionSnapshot.findById(doc._id).lean();
    return res.status(201).json({ success: true, message: "SolutionSnapshot created", data: result });
  } catch (err) {
    try { if (session.inTransaction()) await session.abortTransaction(); } catch (e) { console.error("abortTransaction error:", e); }
    console.error("createSolutionSnapshot error:", err);
 ...
 */
export const getSolutionSnapshot = async (req, res) => {
  try {
    const { id } = req.params;
    if (id) {
      if (!isValidObjectId(id)) return res.status(400).json({ success: false, message: "Invalid ID" });
      const doc = await SolutionSnapshot.findById(id).lean();
      if (!doc) return res.status(404).json({ success: false, message: "SolutionSnapshot not found" });
      return res.status(200).json({ success: true, data: doc });
    }

    if (req.path && req.path.endsWith("/latest")) {
      const latest = await SolutionSnapshot.findOne({}).sort({ createdAt: -1 }).lean();
      if (!latest) return res.status(404).json({ success: false, message: "No SolutionSnapshot found" });
      return res.status(200).json({ success: true, data: latest });
    }

    const items = await SolutionSnapshot.find({}).sort({ createdAt: -1 }).lean();
    return res.status(200).json({ success: true, data: items });
  } catch (err) {
    console.error("getSolutionSnapshot error:", err);
    return res.status(500).json({ success: false, message: err.message });
  }
};

/**
 * Update SolutionSnapshot
 * PATCH /:id
 * Accepts partial updates and supports file upload or URL for snapimg fields
 */
export const updateSolutionSnapshot = async (req, res) => {
  const session = await mongoose.startSession();
  try {
    session.startTransaction();
    ...
    const existing = await SolutionSnapshot.findById(id).session(session);
    if (!existing) {
      if (session.inTransaction()) await session.abortTransaction();
      return res.status(404).json({ success: false, message: "SolutionSnapshot not found" });
    }
    ...
    // simple text fields
    const textFields = [
      "counter1","countertext1","counter2","countertext2","counter3","countertext3",
      "mainhtext","maindtext","snaptext1","snaptext2","snaptext3","snaptext4"
    ];
    ...
    const updated = await SolutionSnapshot.findByIdAndUpdate(id, { $set: setPayload }, { new: true, runValidators: true, session }).lean();

    await session.commitTransaction();
    return res.status(200).json({ success: true, message: "SolutionSnapshot updated", data: updated });
  } catch (err) {
    try { if (session.inTransaction()) await session.abortTransaction(); } catch (e) { console.error("abortTransaction error:", e); }
    console.error("updateSolutionSnapshot error:", err);
    if (err.name === "ValidationError") {
...
/**
 * Delete SolutionSnapshot
 * DELETE /:id
 */
export const deleteSolutionSnapshot = async (req, res) => {
  const session = await mongoose.startSession();
  try {
    session.startTransaction();
    ...
    await SolutionSnapshot.findByIdAndDelete(id, { session });

    await session.commitTransaction();
    return res.status(200).json({ success: true, message: "SolutionSnapshot deleted" });
  } catch (err) {
    try { if (session.inTransaction()) await session.abortTransaction(); } catch (e) { console.error("abortTransaction error:", e); }
    console.error("deleteSolutionSnapshot error:", err);
    return res.status(500).json({ success: false, message: "Internal server error", error: err.message });
  } finally {
    session.endSession();
  }
